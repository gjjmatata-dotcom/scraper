...
st.markdown("<p style='text-align:center;color:#8b949e;font-size:11px;margin:0 0 12px'>"
            "貸借・逆日歩：IRバンク(nisshokin) ／ 信用残：IRバンク(margin) ／ 株価：yfinance</p>",
...
def avg_d(cols, raw, num_cols, dcol):
    a = {c:"" for c in cols}; a[dcol]="【平均】"
    for c in num_cols:
        if c in raw.columns: a[c] = fmt(raw[c].mean(skipna=True))
    return a
...
for tab, (code, info) in zip(tabs, data.items()):
    ...
    with tab:
        ...
        st.markdown("<h3 style='color:#f0f6fc;font-size:14px;margin:0 0 4px'>"
                    "① 貸借取引残高 + 逆日歩（IRバンク・直近順）</h3>", unsafe_allow_html=True)
        st.caption("🔴赤=差引マイナス ／ 🟡橙=平均3倍超 ／ 🟠逆日歩発生 ／ 青=プラス・赤=マイナス")

        if L.empty:
            st.warning("貸借データを取得できませんでした。IRバンクへのアクセスに失敗した可能性があります。")
        else:
            La = L.sort_values("_dt", ascending=True)

            # グラフ：買い残高・売り残高 ＋ 株価ライン（2軸）
        ...
        st.markdown("<h3 style='color:#f0f6fc;font-size:14px;margin:14px 0 4px'>"
                    "② 週次信用残（IRバンク・直近順）</h3>", unsafe_allow_html=True)
        ...
        if M.empty:
            ...
            fig2.add_trace(go.Scatter(x=Ma["日付"],y=Ma["売り残高"],name="売り残高",
                ...
                hovertemplate="%{x}<br>売り残高:%{y:,.0f}<extra></extra>"))
            fig_base(fig2,280); st.plotly_chart(fig2,use_container_width=True)
        ...
        # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        # ③ 株価急変 ＋ 出来高（25日MA・PER・PBR付き）
        # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        st.markdown("<h3 style='color:#f0f6fc;font-size:14px;margin:14px 0 4px'>"
                    "③ 株価急変 ＋ 出来高（直近順）</h3>", unsafe_allow_html=True)
        st.caption("🔴★=大口機関の可能性 ／ 点線=25日MA ／ 🟡=出来高月平均×2超 ／ 青=プラス・赤=マイナス")
        ...
        if P.empty:
            ...
            # 25日MA
            fig3.add_trace(go.Scatter(x=Pa["日付"],y=Pa["MA25"],name="25日MA",
                line=dict(color="#8b949e",width=1.5,dash="dot"),
                hovertemplate="%{x}<br>25日MA:¥%{y:,.1f}<extra></extra>"),row=1,col=1)
            ...
            # 株価テーブル（直近が上・PER・PBR・25日MA乖離率付き）
            raw_p={c:P[c].copy() for c in ["終値","出来高","前日比%","MA25","MA25乖離率","PER","PBR"]}
            ...
            pt["25日MA"]   = raw_p["MA25"].apply(lambda v:f"¥{v:,.1f}" if pd.notna(v) else "-")
            pt["25日乖離率"]= raw_p["MA25乖離率"].apply(lambda v:f"{v:+.2f}%" if pd.notna(v) else "-")
            ...
            sc=["日付","終値","前日比%","25日MA","25日乖離率","PER","PBR","株価判定","出来高","出来高判定"]
            ar={"日付":"【平均】",
                "終値":f"¥{raw_p['終値'].mean():,.1f}",
                "前日比%":f"{raw_p['前日比%'].mean(skipna=True):+.2f}%",
                "25日MA":f"¥{raw_p['MA25'].mean():,.1f}",
                "25日乖離率":f"{raw_p['MA25乖離率'].mean(skipna=True):+.2f}%",
            ...
            def sty_p(row):
                ...
                for c in ["前日比%","25日乖離率"]:
                    if c in cols_l:
                        i=cols_l.index(c)
                        orig=P.loc[P["日付"]==row["日付"],c.replace("25日","MA25")]
                        if c=="前日比%": orig=P.loc[P["日付"]==row["日付"],"前日比%"]
                        elif c=="25日乖離率": orig=P.loc[P["日付"]==row["日付"],"MA25乖離率"]
...
for i,(code,info) in enumerate(data.items()):
    ...
    if not P.empty:
        ...
        ma25_v= fmt(P["MA25乖離率"].iloc[0] if not P.empty else float("nan"),dec=2,suffix="%")
    ...
    with cols4[i%2]:
        ...
        <div style="background:#161b22;border-top:3px solid {c};border-radius:8px;
          ...
          <table style="width:100%;font-size:12px;border-collapse:collapse">
            ...
            <tr><td style="color:#8b949e;padding:2px 0">25日MA乖離率(最新)</td>
                <td style="color:{vc(P['MA25乖離率'].iloc[0]) if not P.empty else '#c9d1d9'};text-align:right">{ma25_v}</td></tr>
